...
declare namespace wx {
	...
	// 阿拉丁数据分析相关方法
	function aldSendEvent(event: string, params: Record<string, string>): any;
	function aldSendOpenid(openId: string): any;
	 ...
	 * 重启当前小程序
	 */
	function restartMiniProgram(object: { success?: () => void; fail?: () => void; complete?: () => void; }): void;
	...
	interface BannerAd {
		...
		/** 取消监听 banner 广告错误事件*/
		offError(callback: (res: { errMsg: string; errCode: 1000 | 1001 | 1002 | 1003 | 1004 | 1005 | 1006 | 1007 | 1008; }) => void): void;
	}
	...
	interface RewardedVideoAd {
		...
		offError(callback: () => void): void;
		/** 监听用户点击 关闭广告 按钮的事件*/
		onClose(callback: (res: { isEnded: boolean; }) => void): any;
		/** 取消监听用户点击 关闭广告 按钮的事件*/
	 ...
	 */
	function createInnerAudioContext(options?: { useWebAudioImplement: boolean; }): InnerAudioContext;
	 ...
	 */
	function onShareMessageToFriend(callback: (res: { success: boolean; errMsg: string; }) => void): any;
	 ...
	 */
	function onInteractiveStorageModified(callback: (key: string) => any): any;

	/**
	 * 取消监听成功修改好友的互动型托管数据事件，该接口在游戏主域使用（基础库 2.14.4 开始支持）
	 * @param callback 事件发生的回调函数，留空则清除所有回调
	 */
	function offInteractiveStorageModified(callback?: Function): any;
	...
	*/
	...
	interface GameIcon {
		load(): any; // 小游戏推荐icon组件加载数据接口。
		show(): any; // 显示小游戏推荐icon组件。
		hide(): any; // 隐藏小游戏推荐icon组件。
		destroy(): any; // 销毁小游戏推荐icon组件，组件销毁后所有方法和事件都将失效。
		onResize(callback: Function): any; // 监听小游戏推荐icon组件位置或者尺寸改变事件。回调函数回抛出一个数组，包含被渲染出来的游戏的信息，该数组为组件实例属性icons的子集。
		offResize(callback: Function): any; // 取消监听小游戏推荐icon组件位置或者尺寸改变事件
		onLoad(callback: Function): any; // 监听小游戏推荐icon组件加载成功事件。
		offLoad(callback: Function): any; // 取消监听小游戏推荐icon组件加载成功事件
		onError(callback: Function): any; // 监听小游戏推荐icon组件加载错误事件。
		offError(callback: Function): any; // 取消监听小游戏推荐icon组件加载错误事件
	}
